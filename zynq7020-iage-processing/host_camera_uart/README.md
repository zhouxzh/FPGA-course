...
本目录是 `sobel_03_uart_hdmi` 的 PC 端图像发送工具。它可以从 USB 摄像头读取画面，或者读取一张本地图片，将图像缩放为 `128x72`，转换成 `RGB888`，再按照约定帧格式通过串口发送到 ZYNQ PS。
...
```text
camera_uart_gui.py
    Tkinter 图形界面版本，推荐学生上机实验使用。

camera_uart_sender.py
    命令行版本，适合教师调试、脚本化测试和问题定位。
...
Baud        = 115200
Input      = Camera 或 Image file
Width       = 128
Height      = 72
FPS         = 0.2
Line delay  = 0.0
Preview     = 勾选
Send once   = 图片模式建议勾选，摄像头模式可不勾选
```
