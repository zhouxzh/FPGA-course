...
`sobel_00_rtl_sim/data/input_rgb.hex` 是固定输入图片，大小为 `128x72`，每个像素 3 个字节，顺序是 `R, G, B`。

`hdmi_common` 是 Sobel 系列工程共用的 HDMI 基础依赖目录，不能删除。`sobel_01_hdmi_pattern`、`sobel_02_hdmi_sobel`、`sobel_03_uart_hdmi` 和 `sobel_04_uart_sobel_hdmi` 的 Vivado 工程中都保留了指向该目录的相对路径，用于记录 HDMI 相关 IP、约束和仿真缓存来源。删除该目录后，Vivado 可能出现 `video_clock`、`rgb2dvi_0`、HDMI 约束或 IP repository 路径缺失的问题。
...
```text
zynq7020-iage-processing/sobel_01_hdmi_pattern/sobel_01_hdmi_pattern.xpr
```
...
```text
zynq7020-iage-processing/sobel_00_rtl_sim/rtl/rgb_to_gray.v
zynq7020-iage-processing/sobel_00_rtl_sim/rtl/sobel_core.v
```
...
```text
zynq7020-iage-processing/sobel_03_uart_hdmi/sobel_03_uart_hdmi.xpr
```
...
```text
zynq7020-iage-processing/sobel_03_uart_hdmi/ps_uart_bram_app/src/main.c
```
...
```text
zynq7020-iage-processing/host_camera_uart/
```
