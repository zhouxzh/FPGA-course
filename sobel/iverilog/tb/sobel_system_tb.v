...
module sobel_system_tb;
    ...
    task write_pgm;
        begin
            pgm_file = $fopen("data/sobel_out.pgm", "w");
            if (pgm_file == 0) begin
                $fatal(1, "Could not open data/sobel_out.pgm");
            end
    ...
    initial begin
        $dumpfile("build/sobel_system_tb.vcd");
        $dumpvars(0, clk);
        $dumpvars(0, rst_n);
    ...
    initial begin
        $readmemh("data/input_rgb.hex", input_rgb);
        ...
        $display("Output image: data/sobel_out.pgm");
        $display("Waveform: build/sobel_system_tb.vcd");
        $finish;
    end
