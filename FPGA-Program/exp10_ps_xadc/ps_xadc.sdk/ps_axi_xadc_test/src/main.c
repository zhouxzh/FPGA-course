...
#define RawToVoltage(AdcData) ((((float)(AdcData))* (9.9f))/4096.0f)
#define RawToCurrent(AdcData) (((float)(AdcData))/((4096.0f) * (0.005f) * (50.0f)))
...
int main()
{
	 ...
	 */
	 ...
	 while(1)
	 {
		 ...
		 raw_data = XSysMon_GetAdcData(&SysMon, XSM_CH_VPVN) ;
		 VpVn = RawToCurrent(raw_data>>4) ;
		 raw_data = XSysMon_GetAdcData(&SysMon, XSM_CH_AUX_MIN+1) ;
		 Vaux1 = RawToVoltage(raw_data>>4) ;
		 raw_data = XSysMon_GetAdcData(&SysMon, XSM_CH_AUX_MIN+9) ;
		 Vaux9 = RawToVoltage(raw_data>>4) ;
		 raw_data = XSysMon_GetAdcData(&SysMon, XSM_CH_AUX_MIN+12) ;
		 Vaux12 = RawToVoltage(raw_data>>4) ;

		 printf("Temperature : %.2fC\t\nVCCINT : %.2fV\t\nVCCAUX : %.2fV\t\n"
				 "VBRAM : %.2fV\t\nVCCPINT : %.2fV\t\nVCCPAUX : %.2fV\t\nVCCPDRO : %.2fV\t\nCurrent : %.2fA\t\nVaux1 : %.2fV\t\nVaux9 : %.2fV\t\nVaux12 : %.2fV\t\n",
				 Temp, vccint, vccaux ,vccbram,vccpint, vccpaux,vccpdro,VpVn, Vaux1,Vaux9,Vaux12) ;
		 sleep(1) ;
	 }
