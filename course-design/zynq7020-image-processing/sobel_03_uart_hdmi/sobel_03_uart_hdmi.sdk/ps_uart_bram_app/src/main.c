...
static int wait_for_sync(u8 first, u8 second, u32 timeout_ms)
{
    u8 prev = 0U;
    u8 cur;
...
static int receive_frame(void)
{
    ...
    if (wait_for_sync(0x55U, 0xaaU, UART_WAIT_MS) != XST_SUCCESS) {
        return -3;
    }
    ...
    for (row_expected = 0U; row_expected < IMG_HEIGHT; row_expected++) {
        ...
        if (wait_for_sync(0x33U, 0xccU, UART_WAIT_MS) != XST_SUCCESS) {
            return -5;
        }
...
int main(void)
{
    ...
    xil_printf("BRAM base: 0x%x, baud: %d\r\n",
               (u32)FRAMEBUFFER_BASEADDR, (u32)UART_BAUD_RATE);
    ...
    while (1) {
        ...
        if (status == 0) {
            frame_count++;
            wait_count = 0U;
            if ((frame_count & 0x1fU) == 0U) {
                xil_printf("received %d frames\r\n", frame_count);
            }
        } else if (status == -3) {
            wait_count++;
            if ((wait_count & 0x3U) == 1U) {
                xil_printf("waiting for frame header\r\n");
            }
        } else {
